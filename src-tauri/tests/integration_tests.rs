...
fn test_latin_word_result_with_verb_forms() {
    ...
    assert_eq!(
        result.part_of_speech,
        "verb - 1st conjugation, present indicative active, 1st person singular"
    );
    assert_eq!(result.definition, "I love");
    assert_eq!(
        result.principal_parts,
        Some("amō, amāre, amāvī, amātum".to_string())
    );
    assert_eq!(
        result.stems,
        Some("Present: amā-, Perfect: amāv-, Participial: amāt-".to_string())
    );
}
...
fn test_error_messages_are_user_friendly() {
    ...
    let error_no_key =
        "API key not configured. Please go to Settings to add your Anthropic API key.";
